...
static char *rev_id = "$Id: test_tpcb_triev2.c,v 1.7 1998/02/21 13:32:39 cessu Exp $";
...
int main(int argc, char *argv[])
{
  ...
  cell_compute_size_stats(GET_ROOT_PTR(test_account));
  cell_compute_size_stats(GET_ROOT_PTR(test_teller));
  cell_compute_size_stats(GET_ROOT_PTR(test_branch));
  cell_fprint_size_stats(stderr);
  return 0;
}
...
static word_t update_branch(word_t old_data, word_t delta, void *dummy)
{
  int i;
  ptr_t new_data = raw_allocate(1 + B_RECORD_SIZE);
  ptr_t p = WORD_TO_PTR(old_data);
...
static word_t update_teller(word_t old_data, word_t delta, void *dummy)
{
  int i;
...
static word_t update_account(word_t old_data, word_t delta, void *dummy)
{
  int i;
				    ...
				    word_t delta)
{
  ...
  SET_ROOT_PTR(test_branch,
	       triev2_insert(GET_ROOT_PTR(test_branch), 
			     scatter_key ? scatter(bid) : bid,
			     32,
			     update_branch, NULL,
			     delta));
  SET_ROOT_PTR(test_teller,
	       triev2_insert(GET_ROOT_PTR(test_teller),
			     scatter_key ? scatter(tid) : tid, 
			     32,
			     update_teller, NULL,
			     delta));
  SET_ROOT_PTR(test_account,
	       triev2_insert(GET_ROOT_PTR(test_account), 
...
#if 1
  SET_ROOT_PTR(test_branch,
	       triev2_replace(GET_ROOT_PTR(test_branch), 
			      scatter_key ? scatter(bid) : bid,
			      32,
			      update_branch, NULL,
			      delta));
  SET_ROOT_PTR(test_teller,
	       triev2_replace(GET_ROOT_PTR(test_teller),
			      scatter_key ? scatter(tid) : tid, 
			      32,
			      update_teller, NULL,
			      delta));
  SET_ROOT_PTR(test_account,
	       triev2_replace(GET_ROOT_PTR(test_account),
			      scatter_key ? scatter(aid) : aid, 
			      32, 
			      update_account, NULL,
			      delta));
...
#if 0
  SET_ROOT_PTR(test_branch,
	       triev2_delete(GET_ROOT_PTR(test_branch), 
			     scatter_key ? scatter(bid) : bid,
			     32,
			     update_branch, NULL,
			     delta));
  SET_ROOT_PTR(test_teller,
	       triev2_delete(GET_ROOT_PTR(test_teller),
			     scatter_key ? scatter(tid) : tid, 
			     32,
			     update_teller, NULL,
			     delta));
  SET_ROOT_PTR(test_account,
	       triev2_delete(GET_ROOT_PTR(test_account), 
			     scatter_key ? scatter(aid) : aid, 
			     32,
			     update_account, NULL,
			     delta));
...
static void insert_records()
{
  ...
  for (key = 0; key < BRANCHES_PER_TPCB * tps; key++) {
    while (!can_allocate(TRIEV2_MAX_ALLOCATION + B_RECORD_SIZE + 1))
      flush_batch();
    ...
    SET_ROOT_PTR(test_branch,
		 triev2_insert(GET_ROOT_PTR(test_branch), skey, 32,
			       NULL, NULL, PTR_TO_WORD(b_record)));
  }

  if (be_verbose)
    fprintf(stdout, "Creating table \"%s\"...\n", TABLE_NAME_TELLER);
  for (key = 0; key < TELLERS_PER_TPCB * tps; key++) {
    while (!can_allocate(TRIEV2_MAX_ALLOCATION + T_RECORD_SIZE + 1))
      flush_batch();
    ...
    SET_ROOT_PTR(test_teller,
		 triev2_insert(GET_ROOT_PTR(test_teller), skey, 32,
			       NULL, NULL, PTR_TO_WORD(t_record)));
  }

  if (be_verbose)
    fprintf(stdout, "Creating table \"%s\"...\n", TABLE_NAME_ACCOUNT);
  for (key = 0; key < ACCOUNTS_PER_TPCB * tps; key++) {
    while (!can_allocate(TRIEV2_MAX_ALLOCATION + A_RECORD_SIZE + 1))
      flush_batch();
#if 0
    if (key % 10000 == 0)
    ...
    assert(triev2_find(GET_ROOT_PTR(test_account), skey, 32) == NULL_WORD);

    SET_ROOT_PTR(test_account,
		 triev2_insert(GET_ROOT_PTR(test_account), skey, 32,
			       NULL, NULL, PTR_TO_WORD(a_record)));
  }
}
...
static void print_table(char *table_name)
{
  ...
  if (history_table) {
    ...
    for (key = 0; key < number_of_records; key++) {
      data = WORD_TO_PTR(triev2_find(root_p, 
