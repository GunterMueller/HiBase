...
static char *rev_id = "$Id: test_tpcb_dh.c,v 1.5 1998/02/21 13:32:30 cessu Exp $";
				    ...
				    word_t delta)
{
  ...
  while (!can_allocate(3 * DH_MAX_ALLOCATION
		       + B_RECORD_SIZE + 1
		       + T_RECORD_SIZE + 1
		       + A_RECORD_SIZE + 1
		       + 2 * 3 + H_RECORD_SIZE + 1)) {
  ...
  SET_ROOT_PTR(test_branch,
	       dh_insert(GET_ROOT_PTR(test_branch), 
			 scatter_key ? scatter(bid) : bid, 
			 update_branch,
			 &delta));

  SET_ROOT_PTR(test_teller,
	       dh_insert(GET_ROOT_PTR(test_teller),
			 scatter_key ? scatter(tid) : tid, 
			 update_teller, 
			 &delta));

  SET_ROOT_PTR(test_account,
	       dh_insert(GET_ROOT_PTR(test_account), 
...
static void insert_records()
{
  ...
  for (key = 0; key < BRANCHES_PER_TPCB * tps; key++) {
    while (!can_allocate(DH_MAX_ALLOCATION + B_RECORD_SIZE + 1))
      flush_batch();
    ...
    SET_ROOT_PTR(test_branch,
		 dh_insert(GET_ROOT_PTR(test_branch), skey,
			   NULL, b_record));
  }
  ...
  for (key = 0; key < TELLERS_PER_TPCB * tps; key++) {
    while (!can_allocate(DH_MAX_ALLOCATION + T_RECORD_SIZE + 1))
      flush_batch();
    ...
    SET_ROOT_PTR(test_teller,
		 dh_insert(GET_ROOT_PTR(test_teller), skey,
			   NULL, t_record));
  }
  ...
  for (key = 0; key < ACCOUNTS_PER_TPCB * tps; key++) {
    while (!can_allocate(DH_MAX_ALLOCATION + A_RECORD_SIZE + 1))
      flush_batch();
#if 0
    if (key % 10000 == 0)
    ...
    assert(dh_find(GET_ROOT_PTR(test_account), skey) == NULL_PTR);

    SET_ROOT_PTR(test_account,
		 dh_insert(GET_ROOT_PTR(test_account), skey,
			   NULL, a_record));
  }
}
...
static void print_table(char *table_name)
{
  ...
  if (history_table) {
    ...
    for (key = 0; key < number_of_records; key++) {
      data = dh_find(root_p, scatter_key ? scatter(key) : key);
      assert(data != NULL_PTR);
      for (i = 1; i <= record_size; i++)
