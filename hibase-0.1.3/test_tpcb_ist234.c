...
static char *rev_id = "$Id: test_tpcb_ist234.c,v 1.4 1998/01/28 15:15:30 siva Exp $";
				    ...
				    word_t amount)
{
  ...
  while (!can_allocate(3 * IST234_MAX_ALLOCATION_IN_INSERT
		       + B_RECORD_SIZE + 1
		       + T_RECORD_SIZE + 1
		       + A_RECORD_SIZE + 1
		       + 2 * 3 + H_RECORD_SIZE + 1)) {
  ...
  SET_ROOT_PTR(test_branch,
	       ist234_insert(GET_ROOT_PTR(test_branch), 
			     new_data,
			     item_cmp, 
			     NULL,
			     update_branch,
  ...
  SET_ROOT_PTR(test_teller,
	       ist234_insert(GET_ROOT_PTR(test_teller),
			     new_data,
			     item_cmp, 
			     NULL,
			     update_teller, 
  ...
  SET_ROOT_PTR(test_account,
	       ist234_insert(GET_ROOT_PTR(test_account), 
			     new_data,
			     item_cmp, 
			     NULL,
			     update_account,
...
static void insert_records()
{
  ...
  for (key = 0; key < BRANCHES_PER_TPCB * tps; key++) {
    while (!can_allocate(IST234_MAX_ALLOCATION_IN_INSERT + B_RECORD_SIZE + 1))
      flush_batch();
    ...
    SET_ROOT_PTR(test_branch,
		 ist234_insert(GET_ROOT_PTR(test_branch), b_record, 
  ...
  for (key = 0; key < TELLERS_PER_TPCB * tps; key++) {
    while (!can_allocate(IST234_MAX_ALLOCATION_IN_INSERT + 2 
			 + T_RECORD_SIZE + 1))
      flush_batch();
    ...
    SET_ROOT_PTR(test_teller,
		 ist234_insert(GET_ROOT_PTR(test_teller), t_record, 
  ...
  for (key = 0; key < ACCOUNTS_PER_TPCB * tps; key++) {
    while (!can_allocate(IST234_MAX_ALLOCATION_IN_INSERT + A_RECORD_SIZE + 1))
      flush_batch();
#if 0
    if (key % 10000 == 0)
...
#if 0
    assert(ist234_find(GET_ROOT_PTR(test_account), a_record, 
		       item_cmp, NULL) == NULL_PTR);
#endif
    SET_ROOT_PTR(test_account,
		 ist234_insert(GET_ROOT_PTR(test_account), a_record, 
...
static void print_table(char *table_name)
{
  ...
  if (history_table) {
	...
	flush_batch();
      ...
      data = ist234_find(root_p, record_p, item_cmp, NULL);
      assert(data != NULL_PTR);
